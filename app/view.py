...
class ServerView(BaseModel):
    ...
    def __repr__(self):
        clients_repr = '\n'.join([f'\t\t{repr(client)}' for client in self.clients]) if self.clients \
            else '\t\tServer has no clients'
        return (f'----------- VeePeeNET {self.veepeenet_version} build {self.veepeenet_build} -----------\n'
        'Xray server info:\n'
