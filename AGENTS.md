...
- `VeePeeNet.stats` stores cumulative `client`, `inbound`, and `outbound` traffic counters. `TrafficStats.__iadd__` and `VeePeeNetStats.__iadd__` are additive merges, so status output should combine persisted and runtime stats rather than replacing one with the other.
...
- **State/statistics helpers in `app/controller/common.py`** — `get_runtime_stats(reset=False)` reads live counters from the Xray API, `get_stored_stats(config)` reads persisted counters from `veepeenet.stats`, `_store_runtime_stats()` persists live counters before stop/restart, and `clear_stats()` resets config stats plus live API stats when the service is running.
