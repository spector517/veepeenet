...
19 commands across 5 modules. All require root; service commands also auto-install the Xray binary if missing.
...
| configure.py | `update-xray [--list] [--limit N] [--version X] [--json]` | Install/update Xray binary; auto-selects latest if no version given |
| state.py | `status [--json]` | Full server view: process state, versions, all clients/rules/outbounds, restart-required flag |
